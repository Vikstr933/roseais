import { pgTable, text, integer, real, timestamp, boolean, serial, jsonb, unique, date } from 'drizzle-orm/pg-core';
...
export const agents = pgTable('agents', {
  ...
  // User-created agents support (added in migration 2027_smart_agent_selection)
  userId: text('user_id').references(() => users.id, { onDelete: 'cascade' }),
  isSystem: integer('is_system').default(1), // 1 = system agent, 0 = user agent
  requiredApiKeys: jsonb('required_api_keys').default([]),
  apiEndpoint: text('api_endpoint'),
  apiConfig: jsonb('api_config').default({}),
});

export const projectRemixes = pgTable('project_remixes', {
  id: serial('id').primaryKey(),
  originalProjectId: integer('original_project_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  remixedProjectId: integer('remixed_project_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  remixedBy: text('remixed_by').notNull().references(() => users.id, { onDelete: 'cascade' }),
  createdAt: timestamp('created_at').defaultNow(),
}, (table) => ({
  uniqueRemix: unique().on(table.originalProjectId, table.remixedProjectId),
}));
...
export const projectVotes = pgTable('project_votes', {
  ...
  uniqueVote: unique().on(table.projectId, table.userId),
}));
...
// Project Members table - tracks users who are members of projects
export const projectMembers = pgTable('project_members', {
  id: serial('id').primaryKey(),
  projectId: integer('project_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  role: text('role').notNull().default('member'), // 'owner', 'admin', 'member', 'viewer'
  permissions: jsonb('permissions').default({}), // JSON permissions object
  joinedAt: timestamp('joined_at').defaultNow(),
  isActive: integer('is_active').notNull().default(1), // INTEGER in Supabase: 1 = active, 0 = inactive
}, (table) => ({
...
export const projectMembersRelations = relations(projectMembers, ({ one }) => ({
  project: one(workspaces, { fields: [projectMembers.projectId], references: [workspaces.id] }),
...
export type InsertAgent = NewAgent; // Alias for backward compatibility
...
export type AssistantSummary = typeof assistantSummaries.$inferSelect;
export type NewAssistantSummary = typeof assistantSummaries.$inferInsert;
...
export const promptChains = pgTable('prompt_chains', {
  ...
  name: text('name').notNull(),
  description: text('description'),
  steps: text('steps').notNull(), // Stored as text in Supabase
...
export const chainExecutions = pgTable('chain_executions', {
  id: serial('id').primaryKey(),
  chainId: integer('chain_id').references(() => promptChains.id, { onDelete: 'cascade' }),
  workspaceId: integer('workspace_id').references(() => workspaces.id, { onDelete: 'set null' }),
  userId: text('user_id').references(() => users.id, { onDelete: 'set null' }),
  status: text('status').notNull(),
  inputData: text('input_data').notNull(),
  outputData: text('output_data'),
  stepResults: text('step_results').notNull(),
  errorMessage: text('error_message'),
  metadata: text('metadata'),
  startedAt: timestamp('started_at').defaultNow(),
  completedAt: timestamp('completed_at'),
...
// Short-term: Recent 15 messages | Long-term: AI-extracted facts
// ============================================================================

// Session tracking for assistants
export const assistantSessions = pgTable('assistant_sessions', {
  id: serial('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  sessionId: text('session_id').notNull(),
  platform: text('platform').notNull().default('discord'), // 'discord', 'web', 'api'
  channelName: text('channel_name'),
  isPrivateDm: boolean('is_private_dm').default(false),
  messageCount: integer('message_count').default(0),
  lastActivity: timestamp('last_activity').defaultNow(),
  createdAt: timestamp('created_at').defaultNow(),
}, (table) => ({
  uniqueSession: unique().on(table.userId, table.sessionId),
}));

// Short-term memory: Recent messages (pruned to last 15)
...
// Long-term memory: AI-extracted facts and knowledge
export const assistantMemories = pgTable('assistant_memories', {
  id: serial('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  category: text('category').notNull(), // 'preference', 'fact', 'contact', 'task', 'context'
  subcategory: text('subcategory'),
  key: text('key').notNull(), // 'girlfriend_name', 'preferred_language'
  value: text('value').notNull(), // 'Elin', 'Swedish'
  confidence: real('confidence').default(0.8),
  source: text('source'), // 'conversation', 'explicit'
  timesReferenced: integer('times_referenced').default(0),
  learnedAt: timestamp('learned_at').defaultNow().notNull(),
  lastUsed: timestamp('last_used').defaultNow(),
  expiresAt: timestamp('expires_at'),
}, (table) => ({
  uniqueMemory: unique().on(table.userId, table.category, table.key),
}));

// Conversation summaries for compression
export const assistantSummaries = pgTable('assistant_summaries', {
  id: serial('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  sessionId: text('session_id').notNull(),
...
// Discord User Mappings - Links Discord user IDs to system user IDs
export const discordUserMappings = pgTable('discord_user_mappings', {
  id: serial('id').primaryKey(),
  discordUserId: text('discord_user_id').unique().notNull(), // Discord user ID (e.g., "123456789012345678")
  discordUsername: text('discord_username'), // Discord username (e.g., "vikstr")
  systemUserId: text('system_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  
  // Verification
  verified: boolean('verified').default(false), // Whether the mapping has been verified
  verificationCode: text('verification_code'), // Optional verification code
  
  createdAt: timestamp('created_at').defaultNow().notNull(),
  lastUsedAt: timestamp('last_used_at').defaultNow(),
}, (table) => ({
  uniqueDiscordUser: unique().on(table.discordUserId),
  uniqueSystemUser: unique().on(table.systemUserId), // One system user can only have one Discord mapping
}));
...
export const discordUserMappingsRelations = relations(discordUserMappings, ({ one }) => ({
  user: one(users, { fields: [discordUserMappings.systemUserId], references: [users.id] }),
}));
...
export type InsertDiscordUserMapping = typeof discordUserMappings.$inferInsert;

// Scheduled emails table for email scheduling functionality
...
export const projectDatabases = pgTable('project_databases', {
  ...
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  projectId: integer('project_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  databaseType: text('database_type').notNull(), // 'mongodb', 'postgresql', 'mysql'
  provider: text('provider').notNull(), // 'supabase', 'neon', 'mongodb-atlas', 'manual'
  connectionString: text('connection_string').notNull(), // Encrypted
  databaseUrl: text('database_url'), // Provider dashboard URL
  status: text('status').notNull().default('active'), // 'active', 'pending', 'failed'
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
}, (table) => ({
  projectIdUnique: unique().on(table.projectId), // One database per project
}));
...
// User usage tracking table (matches Supabase schema)
export const userUsage = pgTable('user_usage', {
  ...
  // Note: Supabase schema doesn't have createdAt, serviceName, or requestType columns
});
