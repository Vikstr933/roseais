-- ==================================================================
-- IMPROVED AGENT SEED DATA
-- Includes Personal Assistant and improved prompts for all agents
-- Uses current Claude models and comprehensive configurations
-- ==================================================================

-- First ensure all required columns exist (from migration 2014)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agents' AND column_name='expertise') THEN
    ALTER TABLE agents ADD COLUMN expertise JSONB DEFAULT '{}'::jsonb;
  END IF;

  -- Add frameworks if missing
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agents' AND column_name='frameworks') THEN
    ALTER TABLE agents ADD COLUMN frameworks JSONB DEFAULT '{}'::jsonb;
  END IF;

  -- Add libraries if missing
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agents' AND column_name='libraries') THEN
    ALTER TABLE agents ADD COLUMN libraries JSONB DEFAULT '{}'::jsonb;
  END IF;

  -- Add best_practices if missing
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agents' AND column_name='best_practices') THEN
    ALTER TABLE agents ADD COLUMN best_practices JSONB DEFAULT '{}'::jsonb;
  END IF;

  -- Add enabled_plugins if missing
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='agents' AND column_name='enabled_plugins') THEN
    ALTER TABLE agents ADD COLUMN enabled_plugins JSONB DEFAULT '[]'::jsonb;
  END IF;
END $$;

-- Clear existing agents and insert improved versions
DELETE FROM agents;

-- ==================================================================
-- 1. PERSONAL ASSISTANT AGENT
-- User's AI-powered productivity companion
-- ==================================================================
...
-- Expert in React/TypeScript architecture design
-- ==================================================================
...
-- Expert React/TypeScript implementation specialist
-- ==================================================================
...
SELECT 'Improved agent seed data inserted successfully!' AS status;
SELECT COUNT(*) || ' agents created with enhanced prompts and current models' AS agent_count FROM agents;
