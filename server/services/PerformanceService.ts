...
export class AdvancedCache {
   ...
   * Useful for invalidating related caches (e.g., all workspace caches)
   */
  deletePattern(pattern: string): number {
    ...
    if (deleted > 0) {
      logger.info(`Cache pattern invalidation: deleted ${deleted} entries matching "${pattern}"`);
    }
    return deleted;
  }
...
export function apiCache(cache: AdvancedCache, ttl?: number) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (cachedResponse) {
      // IMPORTANT: Set CORS headers for cached responses
      // Cached responses bypass CORS middleware, so we need to set headers here
      const origin = req.headers.origin;
      if (origin) {
        // Always allow Vercel and Render origins
