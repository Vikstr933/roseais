...
export interface ModelConfig {
  ...
  enabled: boolean;
  explicitOnly?: boolean; // If true, only use when explicitly requested via preferredModel
}
...
export class MultiModelAIService {
  ...
  private setupModelConfigs() {
    this.modelConfigs = [
      // Anthropic Models
      {
        ...
        maxTokens: 8192,
        temperature: 0.7,
        costPerToken: 0.015, // $15 per 1M tokens (premium model)
        qualityScore: 10.5, // Highest quality for complex reasoning and delegation
        enabled: !!this.anthropic,
        explicitOnly: true // Only use when explicitly requested (e.g., for AnalysisAgent/delegation)
      },
      {
        provider: 'anthropic',
  ...
  private selectBestModel(request: AIRequest): ModelConfig | null {
    ...
    if (request.preferredModel) {
      const preferred = availableModels.find(config => config.model === request.preferredModel);
      if (preferred) {
        logger.info(`Using preferred model: ${request.preferredModel}`);
        return preferred;
      } else {
        logger.warn(`Preferred model ${request.preferredModel} not available, falling back to best available`);
      }
    }

    // Filter out models that require explicit selection (unless they were preferred)
    const selectableModels = availableModels.filter(config => !config.explicitOnly);

    if (selectableModels.length === 0) {
      // Fallback: if no selectable models, use any available (shouldn't happen)
      logger.warn('No selectable models available, using any available model');
