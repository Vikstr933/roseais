...
export class CostMonitoringService {
  ...
  async getCostMetrics(): Promise<CostMetrics> {
    ...
    // Get today's cost (using date column instead of createdAt)
    const dailyCostResult = await db
      .select({ dailyCost: sum(userUsage.cost) })
      .from(userUsage)
      .where(eq(userUsage.date, today.toISOString().split('T')[0]));

    // Get this month's cost (using date column)
    const monthlyCostResult = await db
      .select({ monthlyCost: sum(userUsage.cost) })
      .from(userUsage)
      .where(gte(userUsage.date, monthStart.toISOString().split('T')[0]));
    ...
    // Get cost by service (serviceName doesn't exist in Supabase schema - using userId instead)
    ...
    return {
      ...
      costByService: costByServiceResult.map(service => ({
        serviceName: service.userId || 'unknown', // Using userId since serviceName doesn't exist
  ...
  > {
    ...
    // Using date column instead of createdAt (matches Supabase schema)
    const result = await db
      .select({
        date: userUsage.date,
        cost: sum(userUsage.cost).as('cost'),
        requests: count().as('requests'),
      })
      .from(userUsage)
      .where(gte(userUsage.date, startDate.toISOString().split('T')[0]))
      .groupBy(userUsage.date)
      .orderBy(userUsage.date);

    return result.map(row => ({
      date: row.date || '',
      cost: row.cost != null ? (typeof row.cost === 'string' ? parseFloat(row.cost) : Number(row.cost)) || 0 : 0,
      requests: row.requests != null ? Number(row.requests) : 0,
    }));
  }
  ...
  async getUserCostBreakdown(userId: string): Promise<{
    ...
    // Get daily cost for user (using date column)
    const dailyCostResult = await db
      .select({ dailyCost: sum(userUsage.cost) })
      .from(userUsage)
      .where(
    ...
    // Get monthly cost for user (using date column)
    const monthlyCostResult = await db
      .select({ monthlyCost: sum(userUsage.cost) })
      .from(userUsage)
      .where(
        and(
          eq(userUsage.userId, userId),
          gte(userUsage.date, monthStart.toISOString().split('T')[0])
        )
      );

    // Get cost by date for user (serviceName doesn't exist in Supabase schema)
    const costByDateResult = await db
      .select({
        date: userUsage.date,
    ...
    // Get recent activity (using date instead of createdAt)
    const recentActivityResult = await db
      .select({
        date: userUsage.date,
        cost: userUsage.cost,
        tokensUsed: userUsage.tokensUsed,
        aiRequests: userUsage.aiRequests,
      })
      .from(userUsage)
      .where(eq(userUsage.userId, userId))
      .orderBy(desc(userUsage.date))
    ...
    return {
      ...
      dailyCost: Number(dailyCostResult[0]?.dailyCost) || 0,
      monthlyCost: Number(monthlyCostResult[0]?.monthlyCost) || 0,
      costByService: costByDateResult.map(entry => ({
        serviceName: entry.date || 'unknown', // Using date since serviceName doesn't exist
      ...
      recentActivity: recentActivityResult.map(activity => ({
        ...
        serviceName: 'usage', // Default since serviceName doesn't exist
        requestType: 'ai_request', // Default since requestType doesn't exist
        tokensUsed: activity.tokensUsed || 0,
        aiRequests: activity.aiRequests || 0,
      })),
    };
  }
