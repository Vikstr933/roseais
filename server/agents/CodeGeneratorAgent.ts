...
export class CodeGeneratorAgent extends BaseAgent {
  ...
  async executeTask(task: string | { prompt: string; sharedMemory?: SharedMemory }): Promise<{
    ...
    // ARCHITECTURAL NOTE: Pass raw prompt to AICodeGenerator
    // AICodeGenerator will load all necessary prompts from the database via PromptManager
    // This avoids prompt collision and ensures consistency

    this.logger.info(`Generating code for: ${prompt}`);
    ...
    try {
      ...
      // orchestrated: false means AICodeGenerator will build prompts internally using database
      const aiResponse = await aiCodeGenerator.generateComponent({
        prompt: prompt, // Pass raw prompt - AICodeGenerator handles prompt building via database
        componentName,
        features,
        styling: {
        ...
        orchestrated: false, // Explicit: use internal database prompt building
      });
