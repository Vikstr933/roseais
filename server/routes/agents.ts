...
router.put('/agents/:id', authenticateUser, checkAdminStatus, async (req, res) => {
  try {
    ...
    // Prevent non-admins from updating system agents
    if (isSystemAgent && !isAdmin) {
      console.log(`[FORBIDDEN] User ${userId} cannot update system agent ${id}`);
      return res.status(403).json({
        error: 'Forbidden',
    ...
    // Additional security: Prevent changing isSystem for system agents
    if (req.body.isSystem !== undefined && isSystemAgent) {
      // Non-admins cannot change isSystem status of system agents
      if (!isAdmin) {
        return res.status(403).json({
          error: 'Forbidden',
          message: 'Cannot change isSystem status of system agents'
        });
      }
      // Admins can change it, but log it
      console.log(`[ADMIN] Changing isSystem for system agent ${id} from 1 to ${req.body.isSystem}`);
    }
    ...
    if (systemPrompt !== undefined) {
      // Extra protection: Log systemPrompt changes for system agents
      if (isSystemAgent) {
        console.log(`[SECURITY] System agent ${id} systemPrompt being modified by ${isAdmin ? 'admin' : 'user'} ${userId}`);
      }
    ...
    if (enabledPlugins !== undefined) {
      // enabledPlugins should be an array of strings
      updateData.enabledPlugins = Array.isArray(enabledPlugins) 
        ? enabledPlugins 
        : typeof enabledPlugins === 'string' 
