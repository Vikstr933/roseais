...
// Use the same event emitter as PersonalAssistantAgent
// This ensures events from Discord code generation reach the frontend
let _agentActivityEmitter: EventEmitter | null = null;

const getAgentEventEmitter = (): EventEmitter => {
  if (!_agentActivityEmitter) {
    // Lazy import to avoid circular dependency
    const { agentEventEmitter } = require('../index');
    _agentActivityEmitter = agentEventEmitter;
    _agentActivityEmitter.setMaxListeners(100); // Increase limit for multiple connections
  }
  return _agentActivityEmitter;
};

// Export as object with EventEmitter methods for backwards compatibility
// This allows other files to use agentActivityEmitter.emit() etc.
export const agentActivityEmitter = new Proxy({} as EventEmitter, {
  get(_target, prop) {
    const emitter = getAgentEventEmitter();
    const value = (emitter as any)[prop];
    if (typeof value === 'function') {
      return value.bind(emitter);
    }
    return value;
  }
});
...
router.get('/agent-activity', (req, res) => {
  ...
  // Subscribe to agent events (use 'agent-event' to match PersonalAssistantAgent)
  const emitter = getAgentEventEmitter();
  emitter.on('agent-event', agentListener);
  ...
  req.on('close', () => {
    ...
    getAgentEventEmitter().off('agent-event', agentListener);
    clearInterval(heartbeatInterval);
    res.end();
  });
